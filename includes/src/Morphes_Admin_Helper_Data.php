...
/**
 * Generic helper functions for Morphes_Admin module. This class is a must for any module even if empty.
 * @author Morphes Team
 */
class Morphes_Admin_Helper_Data extends Mage_Core_Helper_Abstract {
	...
	public function getStoreUrl($route, $params = array(), $query = array()) {
		if (!Mage::helper('morphes_admin')->isGlobal()) {
			$params['store'] = Mage::helper('morphes_admin')->getStore()->getId();
		}
		if (count($query)) {
			$params['_query'] = $query;
		}
    ...
    /**
     * @param int $id
     * @return Morphes_Filters_Model_Filter2_Value
     */
    ...
    public function getProductChooserHtml($beforeHtmlCallback = null) {
        ...
        $productsGrid = $layout->createBlock('morphes_admin/chooser_product', '', array(
            'id' => $uniqId,
        ...
        if (!$request->getParam('products_grid')) {
            ...
            $html = $layout->createBlock('adminhtml/template')
                    ->setTemplate('morphes/admin/chooser/product.phtml')
    ...
    public function getCmsBlockChooserHtml($beforeHtmlCallback = null) {
        ...
        $cmsBlocksGrid = $layout->createBlock('morphes_admin/chooser_cmsblock', '', array(
            'id' => $uniqId,
            'use_massaction' => $massAction,
        ));
        ...
        $html = $layout->createBlock('adminhtml/template')
                ->setTemplate('morphes/admin/chooser/cmsblock.phtml')
