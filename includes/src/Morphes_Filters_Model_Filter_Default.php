 ...
 *
 */
class Morphes_Filters_Model_Filter_Default {
		...
		if (!($attribute = $core->collectionFind($this->getFilterableAttributes($model->getStoreId()), 'attribute_code', $model->getCode()))) {
			...
			if ($attribute->isObjectNew()) {
				switch ($model->getCode()) {
					case 'category': $attribute->setIsFilterable(1)->setIsFilterableInSearch(1)->setPosition(-1)->setStoreLabel(Mage::helper('morphes_filters')->__('Category')); break;
					default: throw new Exception(Mage::helper('morphes_filters')->__('Attribute %s not found.', $model->getCode()));
				}
			}
		}
	...
	public function getUseDefaultLabel() {
		return Mage::helper('morphes_filters')->__('Use Product Attribute');
	}
}