...
$session->isLoggedIn()) {
        ...
        $result = array('success' => false);
            ...
            if (empty($login_data['email']) || empty($login_data['pass'])) {
            die('tut');	$result['error'] = Mage::helper('customer')->__('Login and password are required.');
            }
            else
            {
				try
				{
                    $session->login($login_data['email'], $login_data['pass']);
                    $result['success'] = true;
                ...
                catch (Mage_Core_Exception $e)
                {
                    switch ($e->getCode()) {
                        case Mage_Customer_Model_Customer::EXCEPTION_EMAIL_NOT_CONFIRMED:
                            $message = Mage::helper('customer')->__('Email is not confirmed. <a href="%s">Resend confirmation email.</a>', Mage::helper('customer')->getEmailConfirmationUrl($login_data['email']));
                            break;
                        default:
                            $message = $e->getMessage();
                    }
                    $result['error'] = $message;
                    $session->setUsername($login_data['email']);
                }
            }
