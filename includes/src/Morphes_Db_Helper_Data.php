...
/**
 * Generic helper functions for Morphes_Db module. This class is a must for any module even if empty.
 * @author Morphes Team
 */
class Morphes_Db_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getGlobalEntityName($entityName) {
		...
		if (!$core->endsWith($entityName, '_store')) {
			throw new Exception(Mage::helper('morphes_db')->__('Store-level entity %s name should end with _store', $entityName));
		}
	...
	public function replicate($options = array()) {
		...
		try {
	    	...
	    	foreach ($options['targets'] as $entityName => /* @var $target Morphes_Db_Model_Replication_Target */ $target) {
	...
	protected function _getAllReplicationTargets() {
					...
					foreach (Mage::getConfig()->getNode()->global->models->$resourceModel->entities->children() as $entity) {
						if (!empty($entity->replicable)) {
							...
							if (!isset($result[$model])) {
								$result[$model] = Mage::getModel('morphes_db/replication_target')->setEntityName($model);
							}
							...
							foreach (Mage::getResourceSingleton($model)->getReplicationSources() as $source) {
								...
								if (!isset($result[$source])) {
									$result[$source] = Mage::getModel('morphes_db/replication_target')->setEntityName($source);
								}
        ...
        for ($position = 0; $position < $count; $position++) {
            ...
            if ($found) {
                ...
                throw new Exception(Mage::helper('morphes_db')->__('Entities with circular dependencies found: %s', implode(', ', $circular)));
            }
        }
        $this->_orders = $orders;
	...
	public function beginEditing() {
        $this->_lastEditSessionId = Mage::getResourceSingleton('morphes_db/edit_session')->begin();
    ...
    public function beginEditingIfNotAlreadyDoneSo() {
        if (!Mage::helper('morphes_db')->getInEditing()) {
            Mage::helper('morphes_db')->setInEditing();
