...
class SheeplaClient {
    ...
    protected function sendStandard($method, $request) {
        ...
        $this->debug('stream_context_create done', 'sending request to this url: ', $this->config['url'] . $method, 'the request:', $request);
        if (strlen($request) > 0) {
            $result = file_get_contents($this->config['url'] . $method, false, $context);
        } else {
            $result = file_get_contents($this->config['url'] . $method);
        }
     ...
     * @return DOMElement
     */
    protected function arrayToXML($array, $parent) {
        $elements = array();
        foreach ($array as $key => $value) {
            if (is_array($value)) {
                ...
                if (is_numeric($value) || !empty($value)) {
                    //delete & < > chars
                    $elements[] = $this->document->createElement(str_replace(array('&', '<', '>'), ' ', $key), str_replace(array('&', '<', '>'), ' ', $value));
                }
            }
        }
    ...
    protected function arrayToXMLAttribute($array, &$domDocument, &$parent) {
        foreach ($array as $key => $value) {
            if (!empty($value)) {
                ...
                $domAttribute->value = mb_convert_encoding($key, 'utf-8');
                $domElement->appendChild($domAttribute);
                $domAttribute = $domDocument->createAttribute('value');
                $domAttribute->value = mb_convert_encoding($value, 'utf-8');
                $domElement->appendChild($domAttribute);
