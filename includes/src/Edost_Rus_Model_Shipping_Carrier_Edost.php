...
class Edost_Rus_Model_Shipping_Carrier_Edost extends Mage_Usa_Model_Shipping_Carrier_Abstract implements Mage_Shipping_Model_Carrier_Interface {
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request) {
        ...
        if (isset($city_code)) {
            if ($city_code != '0') {
                ...
                Mage::log($dop_tarif_ekonom, true, 'edostlog3.log');
                Mage::log($dop_tarif_mag, true, 'edostlog3.log');
...
class edost_class {
    ...
    var $kod = 0; //????????? ???? 1, ?? ???? win_utf8 (???????? ?????????)
    var $site_win = true; //????????? ???? 1, ?? ???? win_utf8 (???????? ?????????)
    var $index, $curElem;
    var $status = 0;
    var $parser;
    var $rz;
    ...
    function start_tag($parser, $name, $attrs) {
        ...
        switch ($name) {
            case 'tarif': $this->curElem = array();
                break;
            case 'office': $this->curElem = array();
                break;
            default: $this->index = $name;
                break;
        };
    }
    ...
    function char_data($parser, $data) {
        if ((is_array($this->curElem)) && ($this->index)) {
            if (array_key_exists($this->index, $this->curElem))
                $this->curElem[$this->index] = $this->curElem[$this->index] . $data;
            else
                $this->curElem[$this->index] = $data;
        }
        if ((strlen(trim($data)) > 0) and ($this->index == 'stat'))
            $this->status = $data; //?????? ??????????
        if ((strlen(trim($data)) > 0) and ($this->index == 'warning'))
            $this->warning[] = $data; //??????????????
        //if (strlen(trim($data)) > 0) { echo 'String: '.$data.' - '.$this ->index.'<br>'; }  // ??????? ??????
    }
    ...
    function edost_calc($to_city, $weight, $strah, $length, $width, $height, $zip = '') {
        ...
        if (!($weight > 0))
            return( array('qty_company' => 0, 'stat' => 11) );
        if ($to_city == '')
            return( array('qty_company' => 0, 'stat' => 0) );
        ...
        if ($xml == 'Err14')
            return(array('qty_company' => 0, 'stat' => 14));
        if ($xml == '')
            return(array('qty_company' => 0, 'stat' => 8));
        ...
        xml_parser_set_option($this->parser, XML_OPTION_CASE_FOLDING, false); //???? folding ???????, ?? ??? ????? ????? ????? ?????????? ? ??????? ???????

        if (!xml_parse($this->parser, $xml, true)) {
            //die(sprintf('?????? XML: %s ? ?????? %d', xml_error_string(xml_get_error_code($this->parser)),xml_get_current_line_number($this->parser)));
        ...
        //echo '--('.$this -> status.')';
        if ($this->status <> 1)
            return( array('qty_company' => 0, 'stat' => $this->status) );
        ...
        foreach ($this->rz as $n) {
            $i++;
            $rez['price' . $i] = preg_replace("/[^0-9.]/i", "", substr(trim($n['price']), 0, 11)); //???? ????????

            if (!isset($n['day']))
                $n['day'] = '&nbsp;';
            $rez['day' . $i] = substr(trim($n['day']), 0, 60);  //???? ????????
            $rez['company' . $i] = substr($n['company'], 0, 80); //???????? ???????? (? 2 ???? ?????? ??-?? UTF)
            if (!isset($n['name']))
                $n['name'] = '';
            $rez['name' . $i] = substr(trim($n['name']), 0, 80); //???????? ?????? (? 2 ???? ?????? ??-?? UTF)
            $rez['strah' . $i] = substr(trim($n['strah']), 0, 1); //1 - ?? ??????????, 0 - ???
            $rez['id' . $i] = substr(trim($n['id']), 0, 5);   //id

            if (!isset($n['pricecash']))
                $n['pricecash'] = -1;
            $rez['pricecash' . $i] = preg_replace("/[^0-9.-]/i", "", substr(trim($n['pricecash']), 0, 9)); //????? ???????? ??? ?????????? ???????

            if (!isset($n['transfer']))
                $n['transfer'] = 0;
            $rez['transfer' . $i] = preg_replace("/[^0-9.]/i", "", substr(trim($n['transfer']), 0, 9)); //????? ??????? ?? ??????? ??? ?????????
    ...
    function calculateMag($to_city, $weight, $length, $width, $height) {
        ...
        if (($width != 0) && ($weigth != 0) && ($length != 0) && ($height != 0))
