 ...
 * @author Morphes Team
 * Injected into layout instead of standard catalog/layer_filter_attribute in Morphes_Filters_Block_View_Category::_initBlocks.
 */
class Morphes_Filters_Block_Filter extends Mage_Catalog_Block_Layer_Filter_Abstract {
    ...
    public function getSelectedSeoValues() {
        ...
        foreach ($this->getItems() as $item) {
            /* @var $item Morphes_Filters_Model_Item */
    ...
    protected function _initFilter() {
        if ($filter = Mage::getSingleton('morphes_filters/repository')->getFilter($this->getFilterOptions()->getCode())) {
            $this->_filter = $filter;
        }
        else {
            ...
            Mage::getSingleton('morphes_filters/repository')->setFilter($this->getFilterOptions()->getCode(), $this->_filter);
        }
        return $this;
    }
