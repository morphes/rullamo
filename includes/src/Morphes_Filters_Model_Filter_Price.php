 ...
 * @author Morphes Team
 * Injected instead of standard catalog/layer_filter_attribute in Morphes_Filters_Block_Filter_Price constructor.
 */
class Morphes_Filters_Model_Filter_Price
    extends Mage_Catalog_Model_Layer_Filter_Price
    implements Morphes_Filters_Interface_Filter
{
    /**
    ...
    #region Morphes_Filters_Interface_Filter methods
    /**
    ...
    #region common part for all morphes_filters/filter_* models
    /**
     * Creates in-memory representation of a single option of a filter
     * @param array $data
     * @return Morphes_Filters_Model_Item
    ...
    {
        return Mage::getModel('morphes_filters/item')
            ->setData($data)
            ->setFilter($this);
    }
    /**
     * Initializes internal array of in-memory representations of options of a filter
     * @return Morphes_Filters_Model_Filter_Attribute
    ...
    protected function _initItems()
    {
    	...
    	/* @var $ext Morphes_Filters_Helper_Extended */ $ext = Mage::helper(strtolower('Morphes_Filters/Extended'));
    ...
    public function getPriceRange() {
        ...
        if (!$range) {
            if (Mage::helper('morphes_db')->hasOverriddenValueEx($this->getFilterOptions(), 24)) {
                $range = (float)$this->getFilterOptions()->getRangeStep();
            }
            elseif (Mage::helper('morphes_db')->hasOverriddenValueEx($this->getFilterOptions(), 24, 'global_default_mask')) {
    ...
    public function getMSelectedValues() {
        $values = Mage::helper('morphes_core')->sanitizeRequestNumberParam($this->_requestVar, array(
