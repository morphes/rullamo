 ...
 * @method Morphes_Filters_Model_Query setLayer(Mage_Catalog_Model_Layer $value)
 */
class Morphes_Filters_Model_Query extends Varien_Object
{
    ...
    public function addFilter($code, Morphes_Filters_Interface_Filter $model) {
    ...
    public function apply() {
        foreach ($this->_filters as $code => $filter) {
            if (!$filter['isApplyProcessed']) {
                $model = $filter['model'];
                /* @var $model Morphes_Filters_Interface_Filter */
                $model->init();
                if ($filter['isApplied']) {
    ...
    public function getFilterRange($code, $cache = true, $originalCollection = null, $applyFilters = true) {
        ...
        /* @var $currentFilterModel Morphes_Filters_Interface_Filter */
        ...
        if (!$cache || !isset($currentFilter['processedRange'])) {
            ...
            if ($applyFilters) {
                foreach ($this->_filters as $filter) {
                    /* @var $filterModel Morphes_Filters_Interface_Filter */
