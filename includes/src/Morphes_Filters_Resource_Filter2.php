...
/**
 * This resource model handles DB operations with a single model of type Morphes_Filters_Model_Filter2. All 
 * database specific code for Morphes_Filters_Model_Filter2 should go here.
 * @author Morphes Team
 */
class Morphes_Filters_Resource_Filter2 extends Morphes_Db_Resource_Object {
	...
	protected function _construct() {
        $this->_init(strtolower('Morphes_Filters/Filter2'), 'id');
	...
	protected function _addEditedData($object, $fields, $useDefault) {
		Mage::helper('morphes_db')->updateDefaultableField($object, 'is_enabled', Morphes_Filters_Resource_Filter2::DM_IS_ENABLED, $fields, $useDefault);
		Mage::helper('morphes_db')->updateDefaultableField($object, 'display', Morphes_Filters_Resource_Filter2::DM_DISPLAY, $fields, $useDefault);
		Mage::helper('morphes_db')->updateDefaultableField($object, 'name', Morphes_Filters_Resource_Filter2::DM_NAME, $fields, $useDefault);
		Mage::helper('morphes_db')->updateDefaultableField($object, 'is_enabled_in_search', Morphes_Filters_Resource_Filter2::DM_IS_ENABLED_IN_SEARCH, $fields, $useDefault);
		Mage::helper('morphes_db')->updateDefaultableField($object, 'position', Morphes_Filters_Resource_Filter2::DM_POSITION, $fields, $useDefault);
        Mage::helper('morphes_db')->updateDefaultableField($object, 'sort_method', Morphes_Filters_Resource_Filter2::DM_SORT_METHOD, $fields, $useDefault);
        Mage::helper('morphes_db')->updateDefaultableField($object, 'operation', Morphes_Filters_Resource_Filter2::DM_OPERATION, $fields, $useDefault);
        Mage::helper('morphes_db')->updateDefaultableField($object, 'is_reverse', Morphes_Filters_Resource_Filter2::DM_IS_REVERSE, $fields, $useDefault);
    }
    protected function _afterSave(Mage_Core_Model_Abstract $object) {
        if ($edit = $object->getValueData()) {
            foreach ($edit['saved'] as $id => $editId) {
                if ($id > 0) {
                    $editModel = Mage::helper('morphes_admin')->loadModel('morphes_filters/filter2_value', $editId);
                    $data = $editModel->getData();
                    unset($data['id']);
                    unset($data['edit_status']);
                    unset($data['edit_session_id']);
                    $model = Mage::helper('morphes_admin')->loadModel('morphes_filters/filter2_value', $id)->addData($data);
