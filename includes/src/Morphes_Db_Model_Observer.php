 ...
 *
 */
class Morphes_Db_Model_Observer {
	protected function _getProcess() {
		return Mage::getModel('index/process')->load('morphes_db_replicator', 'indexer_code');
	}
	...
	public function afterStoreSave($observer) {
		...
		if (!$dataObject->getdata('_m_prevent_replication')) {
			Mage::helper('morphes_db')->replicate(array(
				'trackKeys' => true,
	...
	public function afterConfigSave($observer) {
		...
		if ($configData->getResourceName() == 'core/config_data' && 
			Mage::helper('morphes_db')->isReplicatedConfigChanged($configData)) 
		{
