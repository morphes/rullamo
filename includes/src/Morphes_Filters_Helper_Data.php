...
/**
 * Generic helper functions for Morphes_Filters module. This class is a must for any module even if empty.
 * @author Morphes Team
 */
class Morphes_Filters_Helper_Data extends Mage_Core_Helper_Abstract {
	/**
	...
	public function getFilterOptionsCollection($allCategories = false) {
	    ...
	    if ($request->getModuleName() == 'catalogsearch' && $request->getControllerName() == 'result' && $request->getActionName() == 'index' ||
	        $request->getModuleName() == 'morphespro_filterajax' && $request->getControllerName() == 'search' && $request->getActionName() == 'index')
	    {
            if (!$this->_filterSearchOptionsCollection) {
                $this->_filterSearchOptionsCollection = Mage::getResourceModel('morphes_filters/filter2_store_collection')
                        ->addColumnToSelect('*')
		...
		if ($allCategories) {
			if (!$this->_filterAllOptionsCollection) {
				$this->_filterAllOptionsCollection = Mage::getResourceModel('morphes_filters/filter2_store_collection')
		        	->addColumnToSelect('*')
		        	->addStoreFilter(Mage::app()->getStore())
		        	->setOrder('position', 'ASC');
			}
			...
			if (!$this->_filterOptionsCollection) {
				Morphes_Core_Profiler::start('mln' . '::' . __CLASS__ . '::' . __METHOD__ . '::' . '$productCollection->getSetIds()');
				$setIds = Mage::getSingleton('catalog/layer')->getProductCollection()->getSetIds();
				Morphes_Core_Profiler::stop('mln' . '::' . __CLASS__ . '::' . __METHOD__ . '::' . '$productCollection->getSetIds()');
				$this->_filterOptionsCollection = Mage::getResourceModel('morphes_filters/filter2_store_collection')
		        	->addFieldToSelect('*')
	...
	public function markLayeredNavigationUrl($url, $routePath, $routeParams) {
        ...
        if ($path == 'catalog/category/view') {
            if (Mage::getStoreConfigFlag('morphes_filters/session/save_applied_filters')) {
                $url .= (strpos($url, '?') === false) ? '?m-layered=1' : '&m-layered=1';
            }
        }
        elseif ($path == 'catalogsearch/result/index') {
            if (Mage::getStoreConfigFlag('morphes_filters/session/save_applied_search_filters')) {
                $url .= (strpos($url, '?') === false) ? '?m-layered=1' : '&m-layered=1';
            }
        }
        else {
            if (Mage::getStoreConfigFlag('morphes_filters/session/save_applied_cms_filters')) {
                $url .= (strpos($url, '?') === false) ? '?m-layered=1' : '&m-layered=1';
            }
        }
		return $url;
    ...
    public function getMode() {
        ...
        elseif (in_array(Mage::helper('morphes_core')->getRoutePath(), array('catalogsearch/result/index', 'morphespro_filterajax/search/index'))) {
            return 'search';
        }
        else {
            return 'category';
    ...
    /**
     * @param Morphes_Filters_Model_Filter2_Store $filterOptions
     * @throws Exception
     * @return bool
     */
    ...
    public function getNoGroups($filterBlocks) {
        ...
        foreach ($filterBlocks as /* @var $filterBlock Morphes_Filters_Block_Filter */ $filterBlock) {
    ...
    /**
     * @param string $field
     * @param Morphes_Filters_Model_Filter2_Store $options
     * @return string
     */
