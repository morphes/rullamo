 ...
 */
class Morphes_Filters_Resource_Filter_Price extends Mage_Catalog_Model_Resource_Eav_Mysql4_Layer_Filter_Price {
...
//        $this->getCount($filter, 1);
    ...
    protected function _getConfigurablePriceFix() {
        if (!Mage::getStoreConfigFlag('morphes_filters/general/adjust_configurable_price')) {
            return '';
        }
        ...
        $values = array();
        foreach (Mage::helper('morphes_filters')->getFilterOptionsCollection() as $filter) {
            if ($filter->getType() == 'attribute' && ($param = $request->getParam($filter->getCode()))) {
                $values = array_merge($values, Mage::helper('morphes_core')->sanitizeNumber($param, array('_')));
            }
        }
        if (count($values) > 0) {
