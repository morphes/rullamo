...
class Novaworks_Blog_Manage_BlogController extends Mage_Adminhtml_Controller_Action {
        ...
        if ($data = $this->getRequest()->getPost()) {
			...
			if(isset($_FILES['post_image']['name']) and (file_exists($_FILES['post_image']['tmp_name']))) {
				try {
					...
					$path = Mage::getBaseDir('media').DS.'novaworks'.DS.'blog'.DS ;
							   
					$result = $uploader->save($path, $_FILES['post_image']['name']);
				 
					$data['post_image'] = 'novaworks'.DS.'blog'.DS.$result['file'];
				}catch(Exception $e) {
