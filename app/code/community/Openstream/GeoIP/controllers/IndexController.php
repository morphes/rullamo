...
if (e == 38 || e == 40) {
    ...
    public function getcityAction() {
        ...
        if (((!isset($session_geo_ip['code']))) || (!isset($model_city['code']))) { 
            if(isset($session_geo_ip['code'])) {
                $info = Mage::getModel('directory/region')->load($session_geo_ip['code'] ); 
            }
            die('change');
        } else {
            if(isset($session_geo_ip['code'])) $code = $session_geo_ip['code'];
            if(isset($model_city['code'])) $code = $model_city['code'];
            $info = Mage::getModel('directory/region')->load( $code );
        } 
        echo $info->getDefaultName(); 
        exit;
    }
}
