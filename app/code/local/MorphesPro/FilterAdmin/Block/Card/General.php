 ...
 *
 */
class MorphesPro_FilterAdmin_Block_Card_General extends Morphes_Admin_Block_Crud_Card_Form implements Mage_Adminhtml_Block_Widget_Tab_Interface {
	/**
    ...
    protected function _prepareForm() {
        ...
        Mage::helper('morphes_core/js')->options('edit-form', array('subforms' => array('#mf_general' => '#mf_general')));
		...
		$fieldset->setRenderer($this->getLayout()->getBlockSingleton('morphes_admin/crud_card_fieldset'));
        ...
        $field = $fieldset->addField('name', 'text', array(
			...
			'name' => 'name',
			'required' => true,
			//'default_bit' => Morphes_Filters_Resource_Filter2::DM_NAME,
			//'default_label' => Mage::helper('morphes_admin')->isGlobal() 
			//	? $this->__('Use Product Attribute') 
			//	: $this->__('Use Product Attribute'),
		));
		$field->setRenderer($this->getLayout()->getBlockSingleton('morphes_admin/crud_card_field'));
		...
		$field = $fieldset->addField('is_enabled', 'select', array(
			...
			//'default_bit' => Morphes_Filters_Resource_Filter2::DM_IS_ENABLED,
			//'default_label' => Mage::helper('morphes_admin')->isGlobal() 
			//	? $this->__('Use Product Attribute') 
        ...
        $field = $fieldset->addField('is_enabled_in_search', 'select', array(
			...
			'required' => true,
			'options' => Mage::getSingleton('morphes_filters/source_filterable')->getOptionArray(),
			//'default_bit' => Morphes_Filters_Resource_Filter2::DM_IS_ENABLED_IN_SEARCH,
			//'default_label' => Mage::helper('morphes_admin')->isGlobal() 
			//	? $this->__('Use Product Attribute') 
			//	: $this->__('Same For All Stores'),
		));
        ...
        $field = $fieldset->addField('display', 'select', array(
			'label' => $this->__('Отображать как'),
			'name' => 'display',
			'required' => true,
			'options' => Mage::getSingleton('morphes_filters/source_display_'.$form->getModel()->getType())->getOptionArray(),
			//'default_bit' => Morphes_Filters_Resource_Filter2::DM_DISPLAY,
			//'default_label' => Mage::helper('morphes_admin')->isGlobal() 
		...
		$field = $fieldset->addField('position', 'text', array(
			...
			'name' => 'position',
			'required' => true,
			'default_bit' => Morphes_Filters_Resource_Filter2::DM_POSITION,
			'default_label' => Mage::helper('morphes_admin')->isGlobal() 
				? $this->__('Use Product Attribute') 
				: $this->__('Same For All Stores'),
		));
        ...
        if ($form->getModel()->getType() == 'attribute') {
           ...
           //     'name' => 'sort_method',
           //     'options' => Mage::getSingleton('morphes_filters/sort')->getOptionArray(),
           //     'required' => true,
           //     'default_bit' => Morphes_Filters_Resource_Filter2::DM_SORT_METHOD,
           //     'default_label' => Mage::helper('morphes_admin')->isGlobal()
           //         ? $this->__('Use System Configuration')
           //         : $this->__('Same For All Stores'),
           // ))->setRenderer($this->getLayout()->getBlockSingleton('morphes_admin/crud_card_field'));

         /*  $fieldset->addField('operation', 'select', array_merge(array(
                'label' => $this->__('Combine Multiple Selections Using'),
                'name' => 'operation',
                'options' => Mage::getSingleton('morphes_filters/operation')->getOptionArray(),
                'required' => true,
            ), Mage::helper('morphes_admin')->isGlobal() ? array() : array(
                'default_bit' => Morphes_Filters_Resource_Filter2::DM_OPERATION,
...
*/
       ...
       //         'label' => $this->__('Reverse Mode'),
        ...
        //        'required' => true,
        //    ), Mage::helper('morphes_admin')->isGlobal() ? array() : array(
        //        'default_bit' => Morphes_Filters_Resource_Filter2::DM_IS_REVERSE,
        //        'default_label' => $this->__('Same For All Stores'),
        //    )))->setRenderer($this->getLayout()->getBlockSingleton('morphes_admin/crud_card_field'));
        }
        $this->setForm($form);
    ...
    public function getAjaxUrl() {
    	return Mage::helper('morphes_admin')->getStoreUrl('*/*/tabGeneral', 
