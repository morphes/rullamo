 ...
 *
 */
class Morphes_Core_Model_Html_Reader {
	protected $_source;
	protected $_state;
	protected $_pos;
	protected $_ch;
	...
	public function setSource($source) {
		$this->_source = $source;
		$this->_state = Morphes_Core_Model_Html_State::INITIAL;
		...
		while ($state != Morphes_Core_Model_Html_State::FINISHED) {
			...
			switch ($state) {
				...
				case Morphes_Core_Model_Html_State::INITIAL:
					if ($ch == ord(' ') || $ch == ord("\r") || $ch == ord("\t") || $ch == ord("\n") || $ch == ord("\f")) {
						if (!$allowWhitespace) {
							...
							throw new Exception(Mage::helper('morphes_core')->__('HTML read error %s: whitespace not expected%s', 
								Morphes_Core_Model_Html_Token::getPosition($token), 
						...
						elseif ($ch == ord('!') || ord('a') <= $ch && $ch <= ord('z') || ord('A') <= $ch && $ch <= ord('Z')) {
							...
							$token['end_pos'] = $this->_pos;
							throw new Exception(Mage::helper('morphes_core')->__('HTML read error %s: unexpected character%s', 
								Morphes_Core_Model_Html_Token::getPosition($token), 
								$this->getSourceAt($token)));
						}
					}
				...
				case Morphes_Core_Model_Html_State::CDATA:
					if ($ch === false) {
						$token['end_pos'] = $this->_pos;
						throw new Exception(Mage::helper('morphes_core')->__('HTML read error %s: unexpected end of text%s', 
							Morphes_Core_Model_Html_Token::getPosition($token), 
							$this->getSourceAt($token)));
					}
