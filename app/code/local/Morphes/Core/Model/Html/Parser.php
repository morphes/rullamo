 ...
 *
 */
class Morphes_Core_Model_Html_Parser extends Varien_Object {
	protected $_token; 
	protected $_openedElements = array();
	
	protected function _construct() {
		$this->_token = $this->getReader()->read($this->hasStartsWith() ? $this->getStartsWith() : Morphes_Core_Model_Html_State::INITIAL_TEXT);
	}
	protected function _read($initialState, $expect = 0, $allowWhitespace = true) {
		...
		if ($expect && $this->_token['type'] != $expect) {
			throw new Exception(Mage::helper('morphes_core')->__('HTML parser error %s: %s expected%s', 
				Morphes_Core_Model_Html_Token::getPosition($this->_token), Morphes_Core_Model_Html_Token::getName($expect), 
	...
	public function parseContent($parentElement = null) {
		...
		while ($this->_token['type'] != Morphes_Core_Model_Html_Token::EOF && $this->_token['type'] != Morphes_Core_Model_Html_Token::TAG_END) {
			switch ($this->_token['type']) {
				case Morphes_Core_Model_Html_Token::TAG_START:
					...
					break;
				case Morphes_Core_Model_Html_Token::CDATA:
					$this->_processCDATA($parentElement, $result, $this->_token);
					$this->_read(Morphes_Core_Model_Html_State::INITIAL_TEXT);
					break;
				case Morphes_Core_Model_Html_Token::COMMENT:
					$this->_processComment($parentElement, $result, $this->_token);
					$this->_read(Morphes_Core_Model_Html_State::INITIAL_TEXT);
					break;
				case Morphes_Core_Model_Html_Token::TEXT:
					$this->_processText($parentElement, $result, $this->_token);
					$this->_read(Morphes_Core_Model_Html_State::INITIAL_TEXT);
					break;
				default: 
					throw new Exception(Mage::helper('morphes_core')->__('HTML parser error %s: unexpected %s%s', 
						Morphes_Core_Model_Html_Token::getPosition($this->_token), 
						Morphes_Core_Model_Html_Token::getName($this->_token['type']), 
						$this->getReader()->getSourceAt($this->_token)));
			}
		}
			...
			case Morphes_Core_Model_Html_Token::TAG_CLOSE: 
				...
				if (!$void) {
					...
					if ($this->_token['type'] != Morphes_Core_Model_Html_Token::TAG_END) {
						throw new Exception(Mage::helper('morphes_core')->__('HTML parser error %s: %s expected%s', 
							Morphes_Core_Model_Html_Token::getPosition($this->_token), 
							Morphes_Core_Model_Html_Token::getName(Morphes_Core_Model_Html_Token::TAG_END), 
							$this->getReader()->getSourceAt($this->_token)));
					}
					$this->_read(Morphes_Core_Model_Html_State::INITIAL, Morphes_Core_Model_Html_Token::NAME, false);
					array_pop($this->_openedElements);
					if ($this->_token['text'] != $elementName) {
						if (in_array($this->_token['text'], $this->_openedElements)) {
							...
							throw new Exception(Mage::helper('morphes_core')->__('HTML parser error %s: closing tag for %s expected%s', 
								Morphes_Core_Model_Html_Token::getPosition($this->_token), $elementName, 
								$this->getReader()->getSourceAt($this->_token)));
						}
					}
					else {
				...
				break;
			default:
				throw new Exception(Mage::helper('morphes_core')->__('HTML parser error %s: %s or %s expected%s', 
					Morphes_Core_Model_Html_Token::getPosition($this->_token), 
					Morphes_Core_Model_Html_Token::getName(Morphes_Core_Model_Html_Token::TAG_SELF_CLOSE), 
					Morphes_Core_Model_Html_Token::getName(Morphes_Core_Model_Html_Token::TAG_CLOSE), 
					$this->getReader()->getSourceAt($this->_token)));
		}
		$this->_read(Morphes_Core_Model_Html_State::INITIAL_TEXT);
