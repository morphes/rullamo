 ...
 *
 */
class Morphes_Core_Resource_Eav extends Mage_Eav_Model_Entity_Abstract {
	 ...
	 * @param Morphes_Core_Model_Eav $model
	...
	protected function _collectSaveData($newObject) {
		...
		if ($newObject->getStoreId()) {
			foreach ($this->getAttributesByCode() as $code => $attribute) {
				if ($attribute->getIsGlobal() == Morphes_Core_Model_Attribute_Scope::_STORE && is_null($newObject->getData($code))) {
