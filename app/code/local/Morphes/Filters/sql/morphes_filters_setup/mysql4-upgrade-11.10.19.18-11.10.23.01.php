...
$installer->run("
	...
	ALTER TABLE `{$this->getTable($table)}`
	  ADD CONSTRAINT `FK_{$this->getTable($table)}_morphes_db/edit_session` FOREIGN KEY (`edit_session_id`) 
	  REFERENCES `{$installer->getTable('morphes_db/edit_session')}` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

	ALTER TABLE `{$this->getTable($table)}`
	  ADD CONSTRAINT `FK_{$this->getTable($table)}_morphes_filters/filter2` FOREIGN KEY (`filter_id`) 
	  REFERENCES `{$installer->getTable('morphes_filters/filter2')}` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
$installer->run("
	...
	ALTER TABLE `{$this->getTable($table)}`
	  ADD CONSTRAINT `FK_{$this->getTable($table)}_morphes_filters/filter2_store` FOREIGN KEY (`filter_id`) 
	  REFERENCES `{$installer->getTable('morphes_filters/filter2_store')}` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
