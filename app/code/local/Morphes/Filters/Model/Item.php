 ...
 * @method Morphes_Filters_Model_Item setMSelected(bool $value)
 * @author Morphes Team
 * Injected instead of standard catalog/layer_filter_item in Morphes_Filters_Model_Filter_Attribute::_createItemEx()
 * method.
 */
class Morphes_Filters_Model_Item extends Mage_Catalog_Model_Layer_Filter_Item {
    /**
    ...
    {
    	// MORPHES BEGIN: add multivalue filter handling
    	...
    	$query = array(
        	// MORPHES BEGIN: save multiple values in URL as concatenated with '_'
	...
	public function getSeoValue() {
        ...
        if (Mage::app()->getRequest()->getParam('m-seo-enabled', true) &&
            ((string)Mage::getConfig()->getNode('modules/MorphesPro_FilterSeoLinks/active')) == 'true' &&
            $this->getFilter()->getMode() != 'search'
        )
        {
            $url = Mage::getModel('morphespro_filterseolinks/url');
            /* @var $url MorphesPro_FilterSeoLinks_Model_Url */
