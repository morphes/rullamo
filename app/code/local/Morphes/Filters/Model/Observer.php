 ...
 *
 */
class Morphes_Filters_Model_Observer {
	...
	public function isConfigChanged($observer) {
		...
		Mage::helper('morphes_db')->checkIfPathsChanged($result, $configData, array(
			'morphes_filters/display/attribute',
			'morphes_filters/display/price',
			'morphes_filters/display/category',
			'morphes_filters/display/decimal',
            'morphes_filters/display/sort_method',
		));
	}
	/**
	...
	public function afterCatalogAttributeSave($observer) {
        ...
        if (!$dataObject->getdata('_m_prevent_replication') && $dataObject->getIsFilterable() == 0) {
            $filter = Mage::getModel('morphes_filters/filter2')->load($dataObject->getAttributeCode(), 'code');
            if ($filter->getId()) {
                $filter->delete();
            }
        }
